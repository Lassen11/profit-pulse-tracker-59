...
export default function ClientsSpasenie() {
  ...
  useEffect(() => {
    if (user) {
      ...
      // Load demo data
      import('@/lib/demoData').then(({ demoBankrotClients }) => {
        const demoClients: ClientData[] = demoBankrotClients.map(client => ({
          clientName: client.full_name,
          contractAmount: client.contract_amount,
          totalPaid: client.total_paid,
          remainingAmount: client.remaining_amount,
          lastPaymentDate: new Date().toISOString().split('T')[0],
          paymentsCount: Math.floor((client.total_paid / client.monthly_payment)),
          transactions: [],
